...
pattern = re.compile(r'<!-- Footer \(Shared Component Type: Footer\) -->.*?</footer>', re.DOTALL)
if pattern.search(content):
    new_content = pattern.sub(premium_footer, content)
    with open('shop.html', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Successfully updated footer in shop.html")
else:
    print("Could not find Footer section in shop.html")
